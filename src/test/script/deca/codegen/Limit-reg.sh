...
# Author : gl49, Teimur
# Initial version : 16/01/2022

# This program is meant to check the
# functionality of the code generation when we
# get low on registers in this case we choose the minimum
# which is 4

cd "$(dirname "$0")"/../../../../.. || exit 1

# We change the paths to execute the tests from the project root.
TESTPATH=src/test/deca/codegen/valid/SansObjet
RESPATH=src/test/results/deca/codegen/valid/SansObjet
TMP=src/test/tmp
