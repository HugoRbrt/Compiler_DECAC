...
TESTPATH=src/test/deca/context/invalid/SansObjet
TMP=src/test/tmp
...
test_context_invalid () {
    ...
    # echo $(test_context $TESTPATH/$1.deca)
    if test_context $TESTPATH/$1.deca 2>&1 | grep -q -e "$1.deca:[0-9][0-9]*:"
        then # normal fail
